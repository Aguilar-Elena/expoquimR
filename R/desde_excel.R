...
#' coshh_desde_excel("mis_datos_coshh.xlsx")
#' }
#'
#' @export
...
#'   columnas de resultado de [inrs_evaluar()].
#'
#' @examples
#' \dontrun{
    ...
    if (!is.na(res$resultado) && res$resultado == "NO DECISION") {
      ...
      if (nrow(sub_add) > 0) {
        ...
        eds  <- vapply(jids, function(j) {
          ...
          une689_ed_jornada(s$concentracion, s$tiempo)
        }, numeric(1))
        eds <- eds[!is.na(eds) & eds > 0]

        if (une689_validar_min_jornadas(length(eds), minimo = 6L)) {
          est <- une689_evaluar_estadistica(eds, vla = vla)
          return(list(
            nombre         = nombre,
            vla            = vla,
